...
def get_patch(*args, patch_size, scale):
    ...
    ret = [
        args[0][iy:iy + ip, ix:ix + ip, :],args[1][iy:iy + ip, ix:ix + ip,:],
        *[a[ty:ty + tp, tx:tx + tp, :] for a in args[2:]]
    ]  # results
    return ret


def get_patch2(*args, patch_size, scale):
    ih, iw = args[0].shape[:2]
    ...
    ret = [
        args[0][iy:iy + ip, ix:ix + ip, :],args[1][iy:iy + ip, ix:ix + ip,:],args[2][iy:iy + ip, ix:ix + ip,:],
